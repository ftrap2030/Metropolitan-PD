...
export default {
  ...
  data: new SlashCommandBuilder()
    .setName('add')
    .setDescription("Give a user access to the ticket you're in")
  ...
  async execute(interaction, config, client) {
    ...
    if (!allowed) return interaction.reply({ content: 'Only the ticket support team can add members.', flags: MessageFlags.Ephemeral });
    const user = interaction.options.getUser('user', true);
    await interaction.channel.permissionOverwrites.edit(user.id, {
      ViewChannel: true, SendMessages: true, ReadMessageHistory: true, AttachFiles: true,
    });
    await interaction.reply({ content: `${user} now has access to this ticket.` });
  },
};
