...
import { getTrainingRecords, getActiveRidealongForUser } from '../../services/leo/trainingService.js';
...
export default {
  ...
  async execute(interaction, config, client) {
    ...
    if (!member) {
      await interaction.reply({ content: 'That user is not currently in this server.', flags: MessageFlags.Ephemeral }).catch(() => {});
      return;
    }

    const [leo, callsigns, shift, loaRecords, trainingRecords, ridealong, hrCases, certifications] = await Promise.all([
      ...
      getTrainingRecords(client, interaction.guildId),
    ...
    const training = Object.values(trainingRecords).filter((record) => String(record.traineeId) === user.id);
    ...
    const activeHr = hrCases.filter((record) => record.kind === 'infraction' && record.status === 'active').length;
